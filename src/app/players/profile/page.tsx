...
import { 
  User, Mail, Phone, MapPin, Trophy, ShieldCheck, LogOut, 
  Shirt, Hash, Layout, Award, CreditCard, ChevronRight, Activity
} from 'lucide-react'
    ...
    return (
      <div className="min-h-screen bg-[#0B1121] flex items-center justify-center">
        <div className="space-y-4 text-center">
          ...
          <p className="text-slate-400 text-sm font-bold uppercase tracking-widest animate-pulse">Loading Workspace...</p>
        </div>
      </div>
    )
  }
      ...
      <nav className="border-b border-white/5 bg-[#0B1121]/80 backdrop-blur-xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/">
              <div className="w-10 h-10 bg-gradient-to-br from-blue-600 to-blue-400 rounded-xl flex items-center justify-center shadow-lg shadow-blue-500/20 cursor-pointer hover:scale-105 transition-all">
                <span className="font-black text-white italic text-lg tracking-tighter">EVL</span>
              </div>
            </Link>
            <span className="text-xl font-bold tracking-tight text-white hidden sm:block">
              ATHLETE <span className="text-blue-400">DASHBOARD</span>
            </span>
          </div>
          ...
          <div className="hidden md:flex items-center gap-8 text-sm font-bold text-slate-400">
            ...
            <Link href="/players" className="hover:text-white transition-colors">DRAFT POOL</Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            >
              ...
              <div className={`absolute top-0 left-0 w-full h-1 ${isSold ? 'bg-yellow-500 shadow-[0_0_15px_rgba(234,179,8,0.6)]' : 'bg-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.6)]'}`} />
              ...
              <div className="mt-8 mb-6 relative">
                <div className={`w-32 h-32 rounded-full overflow-hidden border-2 ${
                  isSold ? 'border-yellow-500 shadow-[0_0_20px_rgba(234,179,8,0.4)]' : 'border-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.4)]'
              ...
              <p className={`text-xs font-black uppercase tracking-widest mt-1 mb-6 ${isSold ? 'text-yellow-400' : 'text-blue-400'}`}>
              ...
              <div className="w-full grid grid-cols-2 gap-4 border-t border-white/5 pt-5 mb-2 text-left">
                ...
                <div className="bg-slate-900/60 border border-white/5 rounded-2xl p-4">
                  ...
                  <span className="text-base font-black text-white font-mono">{player.jersey_size || 'M'}</span>
                </div>
              </div>
            </motion.div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            >
              ...
              <h3 className="text-xs font-black text-slate-400 tracking-widest uppercase mb-4 flex items-center gap-2">
                <Activity className="w-4 h-4 text-blue-500" /> Auction Draft Status
              </h3>
                  ...
                  <div className="p-5 rounded-2xl bg-yellow-500/10 border border-yellow-500/20 flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <h4 className="text-2xl font-black text-white italic uppercase">{player.teams.name}</h4>
                        <p className="text-xs font-medium text-slate-400">Team Franchise Owner: <span className="text-slate-300 font-bold">{player.teams.owner_name}</span></p>
                      </div>
                    </div>
                    <div className="text-right">
                      <span className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-0.5">Purchase Bid</span>
                      <span className="text-3xl font-black text-yellow-400 font-mono">₹{player.sold_price}</span>
                    </div>
                  </div>
                  <p className="text-sm text-slate-400 leading-relaxed">
                    Congratulations! You have been successfully drafted into the **{player.teams.name}** roster for EVL Season 1. Your training slot schedules and matches details will update directly here.
                  </p>
                </div>
              )}
              ...
              {inAuction && (
                <div className="p-6 rounded-2xl bg-red-500/10 border border-red-500/20 flex items-center justify-between">
                  <div className="space-y-1">
                    ...
                    <h4 className="text-xl font-black text-white italic uppercase mt-2">Active Roster Bidding</h4>
                    <p className="text-xs text-slate-400">Teams are currently bidding on your profile. Watch the Live Center page!</p>
                  </div>
                  <Link href="/">
                ...
                <div className="space-y-4">
                  <div className="p-5 rounded-2xl bg-blue-500/10 border border-blue-500/20 flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div>
                      ...
                      <h4 className="text-xl font-black text-white italic uppercase mt-1">Ready for Auction Draft</h4>
                      <p className="text-xs text-slate-400 mt-1">You are verified and in the auction registry. Draft commences shortly.</p>
                    </div>
                    <div className="text-right">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              >
                <h3 className="text-xs font-black text-slate-400 tracking-widest uppercase mb-5 flex items-center gap-2">
                  <User className="w-4 h-4 text-blue-500" /> Personal & Athletic Specs
                </h3>
                ...
                <div className="space-y-4 text-sm">
                  <div className="flex justify-between py-2.5 border-b border-white/5">
                    ...
                    <span className="text-slate-200 font-semibold">{player.age || '—'} Years</span>
                  </div>
                  <div className="flex justify-between py-2.5 border-b border-white/5">
                    <span className="text-slate-500 font-bold uppercase tracking-wider text-[10px]">Location Wing/Flat</span>
                    <span className="text-slate-200 font-semibold">{player.wing_building || '—'}</span>
                  </div>
                  <div className="flex justify-between py-2.5 border-b border-white/5">
                    <span className="text-slate-500 font-bold uppercase tracking-wider text-[10px]">Phone Number</span>
                    <span className="text-slate-200 font-semibold">{player.phone_number || '—'}</span>
                  </div>
                  <div className="flex justify-between py-2.5 border-b border-white/5">
                    <span className="text-slate-500 font-bold uppercase tracking-wider text-[10px]">Email Address</span>
                    <span className="text-slate-200 font-semibold truncate max-w-[180px]">{player.email || '—'}</span>
                  </div>
                  <div className="flex justify-between py-2.5">
                    <span className="text-slate-500 font-bold uppercase tracking-wider text-[10px]">Verification UTR</span>
                    <span className="text-slate-200 font-mono font-semibold">{player.utr_number || '—'}</span>
                  </div>
                </div>
              </motion.div>
              ...
              >
                <h3 className="text-xs font-black text-slate-400 tracking-widest uppercase mb-5 flex items-center gap-2">
                  <Shirt className="w-4 h-4 text-blue-500" /> Jersey & Experience Specs
                </h3>

                <div className="space-y-4 text-sm">
                  <div className="flex justify-between py-2.5 border-b border-white/5">
                    <span className="text-slate-500 font-bold uppercase tracking-wider text-[10px]">Jersey Back Name</span>
                    <span className="text-slate-200 font-semibold">{player.jersey_name || '—'}</span>
                  </div>
                  <div className="flex justify-between py-2.5 border-b border-white/5">
                    <span className="text-slate-500 font-bold uppercase tracking-wider text-[10px]">Jersey Chosen Size</span>
                    <span className="text-slate-200 font-semibold">{player.jersey_size || '—'}</span>
                  </div>
                  <div className="flex justify-between py-2.5 border-b border-white/5">
                    <span className="text-slate-500 font-bold uppercase tracking-wider text-[10px]">Jersey Number</span>
                    <span className="text-slate-200 font-semibold font-mono">#{player.jersey_number || '—'}</span>
                  </div>
                  <div className="flex justify-between py-2.5 border-b border-white/5">
                    <span className="text-slate-500 font-bold uppercase tracking-wider text-[10px]">Volleyball Position</span>
                    <span className="text-slate-200 font-semibold">{player.playing_position || '—'}</span>
                  </div>
                  <div className="flex justify-between py-2.5">
                    <span className="text-slate-500 font-bold uppercase tracking-wider text-[10px]">Experience Level</span>
                    <span className="text-slate-200 font-semibold truncate max-w-[180px]">{player.volleyball_experience || '—'}</span>
                  </div>
                </div>
              </motion.div>
            ...
            {(player.volleyball_experience || player.previous_tournament_experience) && (
              <motion.div
              ...
              >
                <h4 className="text-[10px] font-black uppercase text-slate-500 tracking-widest flex items-center gap-1.5"><Award className="w-3.5 h-3.5 text-blue-500" /> Career & Tournament Experience</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                  <div>
                    <span className="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-0.5">Playing Experience</span>
                    <p className="text-slate-200 text-xs leading-relaxed">{player.volleyball_experience || 'No profile logs'}</p>
                  </div>
                  <div>
                    <span className="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-0.5">Tournament Background</span>
                    <p className="text-slate-200 text-xs leading-relaxed">{player.previous_tournament_experience || 'No previous tournament logs'}</p>
                  </div>
                </div>
              </motion.div>
            )}
