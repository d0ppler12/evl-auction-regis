        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-accent to-blue-400 rounded-xl flex items-center justify-center shadow-lg shadow-accent/20">
              <span className="font-black text-primary italic text-lg tracking-tighter">EVL</span>
            </div>
          ...
          <div className="hidden md:flex items-center gap-8 text-sm font-bold text-muted">
            ...
            <Link href="/points-table" className="hover:text-primary transition-colors">POINTS TABLE</Link>
          </div>
      ...
      <main className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <section className="card-base relative overflow-hidden">
          ...
          <div className="relative z-20 p-8 md:p-16 flex flex-col md:flex-row items-center justify-between gap-12">
            <div className="flex-1 max-w-2xl">
              ...
              <h1 className="text-5xl md:text-7xl font-black text-primary leading-[1.1] tracking-tighter mb-6 italic">
                RISE.<br/>
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-accent to-blue-200">SPIKE.</span><br/>
                DOMINATE.
              </h1>
              ...
              <div className="flex flex-wrap gap-4">
                <Link href="/register" className="px-8 py-4 rounded-xl bg-accent hover:bg-blue-400 text-background font-bold shadow-[0_0_30px_rgba(96,165,250,0.3)] transition-all hover:-translate-y-1">
                  REGISTER AS PLAYER
                </Link>
          ...
          <div className="relative z-20 border-t border-white/5 bg-surface/40 backdrop-blur-md px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <button className="text-accent hover:text-blue-300 font-bold text-sm flex items-center gap-1 w-full md:w-auto justify-center md:justify-end">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="xl:col-span-2 space-y-6">
            ...
            <section className="card-base p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-black tracking-tight text-primary">TOP TEAMS</h2>
                <Link href="/teams" className="text-xs font-bold text-accent hover:text-blue-300 uppercase tracking-wider flex items-center">
                  View All Teams <ChevronRight className="w-4 h-4 ml-1" />
                </Link>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                {topTeams.map((team) => (
                  <div key={team.id} className="card-base card-hover p-4 flex flex-col items-center text-center cursor-pointer group">
                    <div className="w-12 h-12 md:w-16 md:h-16 bg-surface rounded-full mb-3 flex items-center justify-center border border-white/10 shadow-lg group-hover:scale-110 transition-transform">
                      <span className="text-xl md:text-2xl font-black text-primary">{team.logo}</span>
                    </div>
                    <span className="font-bold text-primary text-xs md:text-sm mb-2">{team.name}</span>
                    <div className="w-full flex justify-between text-[10px] text-muted uppercase font-bold border-t border-white/5 pt-2">
                      <div className="flex flex-col items-start">
                        <span>Captain</span>
                        <span className="text-secondary">{team.captain.split(' ')[0]}</span>
                      </div>
            ...
            <section className="card-base p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-black tracking-tight text-primary">UPCOMING MATCHES</h2>
          ...
          <div className="xl:col-span-1">
            ...
            <section className="card-base p-6 h-full overflow-hidden flex flex-col">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-black tracking-tight text-primary">POINTS TABLE</h2>
                <div className="flex gap-2">
                  <button className="px-3 py-1 bg-accent/20 border border-accent/30 text-accent text-[10px] font-bold rounded hover:bg-accent hover:text-background transition-colors">ALL TEAMS</button>
                </div>
              </div>
                  ...
                  <tbody>
                    {pointsTable.map((row) => (
                      <tr key={row.pos} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                        <td className="py-3 px-2 font-bold text-primary">{row.pos}</td>
                        <td className="py-3 px-2 font-bold flex items-center gap-2">
                          <div className="w-5 h-5 bg-surface rounded-full flex items-center justify-center text-[8px] border border-white/10 hidden sm:flex text-primary">{row.team[0]}</div>
                          <span className="truncate max-w-[80px] sm:max-w-none text-secondary">{row.team}</span>
                        </td>
                        <td className="py-3 px-2 text-center text-muted">{row.p}</td>
                        <td className="py-3 px-2 text-center text-muted">{row.w}</td>
                        <td className="py-3 px-2 text-center text-muted">{row.l}</td>
                        <td className="py-3 px-2 text-center font-bold text-accent">{row.pts}</td>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          {/* Player Spotlight */}
          <section className="xl:col-span-1 card-base overflow-hidden flex flex-col">
            <div className="p-6 border-b border-white/5 flex justify-between items-center bg-surface/50">
              <h2 className="text-xl font-black tracking-tight text-primary">PLAYER SPOTLIGHT</h2>
              <Link href="/players" className="text-[10px] font-bold text-accent uppercase hover:text-blue-300">View Profile</Link>
            </div>
            <div className="relative p-6 flex-grow flex flex-col justify-end min-h-[300px]">
              <div className="absolute bottom-0 right-0 w-48 h-48 bg-accent/20 blur-[50px] rounded-full"></div>
              {/* Silhouette Placeholder - fallback if no player image */}
              <div className="absolute bottom-0 right-4 w-40 h-56 bg-gradient-to-t from-background to-transparent opacity-50 mix-blend-screen pointer-events-none"></div>
              
              <div className="relative z-10">
                <div className="flex items-end gap-3 mb-1">
                  <span className="text-4xl font-black text-muted italic leading-none">#10</span>
                  <h3 className="text-3xl font-black text-primary italic leading-none">ARYAN<br/>MISHRA</h3>
                </div>
                <p className="text-accent font-bold text-sm mb-6 uppercase tracking-widest">Outside Hitter</p>
                
                <div className="grid grid-cols-2 gap-4 text-xs font-bold text-muted mb-6 border-l-2 border-accent pl-4">
                  <div>AGE <span className="text-primary block text-sm">22</span></div>
                  <div>HEIGHT <span className="text-primary block text-sm">198 CM</span></div>
                  <div>HAND <span className="text-primary block text-sm">RIGHT</span></div>
                  <div>MVP <span className="text-primary block text-sm">3</span></div>
                </div>

                <div className="grid grid-cols-4 gap-2 text-center bg-surface/50 p-3 rounded-xl border border-white/5">
                  <div>
                    <div className="text-[10px] text-muted mb-1">ATTACK %</div>
                    <div className="font-bold text-primary">48.6</div>
                  </div>
                  <div>
                    <div className="text-[10px] text-muted mb-1">BLOCKS</div>
                    <div className="font-bold text-primary">32</div>
                  </div>
                  <div>
                    <div className="text-[10px] text-muted mb-1">ACES</div>
                    <div className="font-bold text-primary">18</div>
                  </div>
                  <div>
                    <div className="text-[10px] text-muted mb-1">DIGS</div>
          ...
          {/* Live Match Center */}
          <section className="xl:col-span-2 card-base p-6 flex flex-col relative overflow-hidden">
            ...
            <div className="flex justify-between items-center mb-8 relative z-10">
              <button className="text-xs font-bold text-muted flex items-center hover:text-primary"><ChevronLeft className="w-4 h-4"/> BACK</button>
              <h2 className="text-xl font-black tracking-tight text-primary hidden sm:block">LIVE MATCH CENTER</h2>
              <div className="text-xs font-bold text-muted flex items-center gap-1"><MapPin className="w-3 h-3"/> Eternia Arena</div>
            </div>
            ...
            <div className="grid grid-cols-5 gap-2 border-y border-white/5 py-4 mb-8 relative z-10 text-center">
              <div><div className="text-[10px] text-muted mb-1">SET 1</div><div className="font-bold text-secondary text-sm">25-20</div></div>
              <div><div className="text-[10px] text-accent mb-1">SET 2</div><div className="font-bold text-sm text-accent">24-21</div></div>
              <div><div className="text-[10px] text-muted mb-1">SET 3</div><div className="font-bold text-sm text-muted">-</div></div>
              <div><div className="text-[10px] text-muted mb-1">SET 4</div><div className="font-bold text-sm text-muted">-</div></div>
              <div><div className="text-[10px] text-muted mb-1">SET 5</div><div className="font-bold text-sm text-muted">-</div></div>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-6 relative z-10 flex-grow">
               ...
               <div className="w-full md:w-48 flex flex-col justify-end gap-3">
                  <button className="w-full py-3 rounded-xl border border-white/10 hover:bg-white/5 font-bold text-sm text-primary transition-colors">MATCH STATS</button>
                  <button className="w-full py-3 rounded-xl bg-gold hover:bg-yellow-400 font-bold text-sm text-background flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(250,204,21,0.3)] transition-all">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 pb-12">
          ...
          <section className="xl:col-span-2 card-base p-6">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-black tracking-tight text-primary">UPCOMING SCHEDULE</h2>
          ...
          <section className="xl:col-span-1 relative rounded-2xl overflow-hidden shadow-2xl border border-accent/30 group bg-surface">
             ...
             <div className="relative z-20 p-8 flex flex-col items-center justify-center h-full text-center min-h-[300px]">
                ...
                <h3 className="text-3xl font-black text-primary italic mb-2">REGISTER NOW</h3>
                <p className="text-accent text-sm font-medium mb-8">BE PART OF EVL SEASON 1</p>
                <Link href="/register" className="w-full py-4 bg-gold hover:bg-yellow-400 text-background font-black uppercase tracking-widest rounded-xl shadow-[0_0_20px_rgba(250,204,21,0.4)] transition-all hover:scale-105 active:scale-95 text-sm">
                  REGISTER AS PLAYER
                </Link>
             </div>
          </section>
