        ...
        <div className="bg-slate-900/50 border border-white/10 rounded-2xl p-6 backdrop-blur-sm space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <input
              ...
              value={form.wing_building}
              onChange={(e) =>
                setForm({ ...form, wing_building: e.target.value })
              }
              className="bg-slate-800 border border-white/10 rounded-xl px-4 py-3 text-white"
            />
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, jersey_name: e.target.value })
              }
              className="bg-slate-800 border border-white/10 rounded-xl px-4 py-3 text-white"
            />
            <select
      ...
      {loading ? (
        ...
        <div className="bg-slate-900/50 border border-white/5 rounded-2xl p-12 backdrop-blur-sm text-center">
          ...
          <p className="text-slate-400">
            Add players manually or review registrations.
          </p>
        </div>
      ) : (
            ...
            >
              ...
              {p.auction_status === "sold" ? (
                <p className="text-xs font-bold text-blue-400 uppercase">
                  SOLD TO {p.teams?.name || "TEAM"} ·{" "}
                  {p.sold_price?.toLocaleString()} pts
                </p>
              ) : (
                <p className="text-xs text-slate-500 uppercase">
                  {p.status} · {p.base_price?.toLocaleString()} pts
                </p>
              )}
            </div>
          ))}
