  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId) {
        ...
        setMatches((prev) =>
          prev.map((m) =>
            m.id === editingId ? { ...m, ...(updated as any) } : m,
          ),
        );
      } else {
        ...
        <div className="bg-slate-900/50 border border-white/10 rounded-2xl p-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {form.match_type === "knockout" && (
              <select
                value={form.bracket_round}
                onChange={(e) =>
                  setForm({ ...form, bracket_round: e.target.value })
                }
                className="bg-slate-800 border border-white/10 rounded-xl px-4 py-3 text-white"
              >
            ...
            <input
              type="number"
              placeholder="Sets Team A"
              value={form.sets_team_a}
              onChange={(e) =>
                setForm({ ...form, sets_team_a: e.target.value })
              }
              className="bg-slate-800 border border-white/10 rounded-xl px-4 py-3 text-white"
            />
            <input
              type="number"
              placeholder="Sets Team B"
              value={form.sets_team_b}
              onChange={(e) =>
                setForm({ ...form, sets_team_b: e.target.value })
              }
              className="bg-slate-800 border border-white/10 rounded-xl px-4 py-3 text-white"
            />
          </div>
          <div className="flex gap-3">
            ...
            <button
              ...
              className="bg-slate-700 text-white font-bold py-2 px-6 rounded-xl"
            >
              Cancel
            </button>
          </div>
      ...
      {loading ? (
        ...
        <div className="bg-slate-900/50 border border-white/5 rounded-2xl p-12 backdrop-blur-sm text-center">
          ...
          <p className="text-slate-400">
            Create the first fixture to get started.
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {matches.map((m) => (
            <div
              key={m.id}
              className="bg-slate-900/50 border border-white/5 rounded-2xl p-5 flex justify-between items-center"
            >
              <div>
                ...
                <div className="flex gap-2 items-center mt-1">
                  <span
                    className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${m.match_type === "knockout" ? "bg-amber-500/20 text-amber-400 border border-amber-500/30" : "bg-blue-500/20 text-blue-400 border border-blue-500/30"}`}
                  >
                    {m.match_type === "knockout"
                      ? m.bracket_round?.replace("_", " ") || "Knockout"
                      : "League"}
                  </span>
                  <span
                    className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${m.status === "live" ? "bg-red-500/20 text-red-400 border border-red-500/30 animate-pulse" : "bg-slate-800 text-slate-400 border border-white/10"}`}
                  >
                    {m.status}
                  </span>
                </div>
