...
export default function TeamsManagement() {
  ...
  const openEdit = (team: any) => {
    ...
    setForm({
      ...
      purse_remaining: String(
        team.purse_remaining ?? team.total_purse ?? 100000,
      ),
  ...
  const handleSave = async () => {
    if (!form.name || !form.owner_name)
      return alert("Name and owner are required");
    setSaving(true);
    try {
      let team: any;
      const payload = {
  ...
  const handleReleasePlayer = async (teamId: string, playerId: string) => {
    if (
      !confirm(
        "Are you sure you want to release this player? Their sold price will be refunded to the team and they will return to the auction pool.",
      )
    )
      return;
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button
          onClick={openAdd}
          className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-2 px-4 rounded-xl flex items-center gap-2 transition-all"
        >
        ...
        <div className="bg-slate-900/50 border border-white/10 rounded-2xl p-6 backdrop-blur-sm space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input
              ...
              className="bg-slate-800 border border-white/10 rounded-xl px-4 py-3 text-white"
            />
            <input
            ...
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, total_purse: e.target.value })
              }
              className="bg-slate-800 border border-white/10 rounded-xl px-4 py-3 text-white"
            />
            <input
              type="number"
              placeholder="Purse Remaining"
              value={form.purse_remaining}
              onChange={(e) =>
                setForm({ ...form, purse_remaining: e.target.value })
              }
              className="bg-slate-800 border border-white/10 rounded-xl px-4 py-3 text-white"
            />
            <input
              placeholder="Color Theme (hex)"
              value={form.color_theme}
              onChange={(e) =>
                setForm({ ...form, color_theme: e.target.value })
              }
              className="bg-slate-800 border border-white/10 rounded-xl px-4 py-3 text-white"
            />
            <select
              ...
              className="bg-slate-800 border border-white/10 rounded-xl px-4 py-3 text-white outline-none"
            >
            ...
            <label className="flex items-center gap-2 text-white text-sm col-span-full">
              <input
                type="checkbox"
                checked={form.is_playing_owner}
                onChange={(e) =>
                  setForm({ ...form, is_playing_owner: e.target.checked })
                }
              />
              Playing Owner
            </label>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="bg-slate-700 hover:bg-slate-600 text-white font-bold py-2 px-6 rounded-xl"
            >
              Cancel
            </button>
          </div>
      ...
      {loading ? (
        ...
        <div className="bg-slate-900/50 border border-white/5 rounded-2xl p-12 backdrop-blur-sm text-center">
          ...
          <p className="text-slate-400">
            Add teams to start managing their squads and budgets.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {teams.map((team) => (
            <div
              key={team.id}
              className="bg-slate-900/50 border border-white/5 rounded-2xl p-5 backdrop-blur-sm"
            >
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  {team.logo_url ? (
                    ...
                    <div className="w-12 h-12 bg-blue-600/20 rounded-xl flex items-center justify-center text-blue-400 font-black">
                      {team.name?.charAt(0)}
                    </div>
                  )}
                  <div>
                ...
                {team.players && team.players.length > 0 ? (
                  <div className="space-y-2 max-h-[150px] overflow-y-auto pr-1">
                    {team.players.map((p: any) => (
                      <div
                        key={p.id}
                        className="flex justify-between items-center bg-slate-800/50 p-2 rounded-lg border border-white/5"
                      >
